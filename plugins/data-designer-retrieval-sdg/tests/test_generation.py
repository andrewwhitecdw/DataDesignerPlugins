...
@pytest.mark.parametrize("nonpositive_buffer_size", [0, -1])
def test_run_generation_rejects_nonpositive_buffer_size(
    tmp_path: Path, nonpositive_buffer_size: int
) -> None:
    with pytest.raises(ValueError, match="buffer_size"):
        generation.run_generation(
            GenerationRunConfig(
                seed_source=DocumentChunkerSeedSource(path=str(tmp_path)),
                output_dir=tmp_path / "output",
                buffer_size=nonpositive_buffer_size,
            )
        )
